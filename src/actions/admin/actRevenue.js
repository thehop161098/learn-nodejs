...
//
export function actGetWalletParent() {
    return dispatch => {
        return callApi(`${URL}/getBankParents`).then(res => {
            if (res && res.data) {
                if (res.data.banks) return res.data.banks;
                return [];
            }
        });
    }
...
export function actAutoCompleteRequest(data) {
    return dispatch => {
        return callApi(`${URL}/getAutoComplete`, 'POST', data).then(res => {
            if (res && res.data) {
                let result = res.data;
                return result;
            }
