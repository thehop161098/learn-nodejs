...
import * as actEmail from '../../actions/admin/actEmail';
import {
    openForm, closeForm,
...
class EmailContainer extends Component {
    ...
    componentDidMount() {
        ...
        let roles_access = this.props.roles['EmailContainer'] ? this.props.roles['EmailContainer'] : [];
    ...
    onUpdateStatus = (_id, status) => {
        if (!checkPermission(this.state.roles_access, 'update')) return;
        if (_id !== "") {
            return this.props.actEditStatusRequest(_id, status).then(res => {
                if (res.success) {
                    showToast(res.message);
                } else {
                    showToast(res.message, 'error');
...
const mapProps = merge(actEmail, actIndex);
export default connect(mapState, mapProps)(EmailContainer);