...
import * as actRevenue from '../../actions/admin/actRevenue';
import {
    openForm, closeForm,
...
class RevenueContainer extends Component {
    ...
    componentDidMount() {
        ...
        let roles_access = this.props.roles['RevenueContainer'] ? this.props.roles['RevenueContainer'] : [];
    ...
    onAutoComplete = (data) => {
        return this.props.actAutoCompleteRequest(data);
    }
    ...
    render() {
        return (
            <RevenuePage
                {...this.state}
                {...this.props}
                onGetWalletParent={this.props.actGetWalletParent}
...
const mapProps = merge(actRevenue, actIndex);
export default connect(mapState, mapProps)(RevenueContainer);